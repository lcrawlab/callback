...
cluster_optimal_louvain_resolution_parameter <- function(seurat_obj,
  ...
  # todo move this function into  flow so that it happens automatically and
  # doesn't repeat computation
  ...
  all_genes <- rownames(new_seurat_obj)

  new_seurat_obj <- Seurat::ScaleData(new_seurat_obj, features = all_genes)
...
compute_knockoff_filter_one_cluster <- function(seurat_obj, cluster, q) {
  ...
  ret <- as.data.frame(list("selected_gene" = selected_genes,
                            "W" = selected_Ws))
...
FindMarkersWithKnockoffs <- function(seurat_obj,
                                     ident.1,
                                     ident.2,
                                     q,
                                     num_var_features) {
  ...
  markers <- compute_knockoff_filter(new_seurat_obj,
                                     cluster_labels[1],
                                     cluster_labels[2],
                                     q)
...
compare_markers_jaccard <- function(orig_seurat_obj, knock_seurat_obj, q,
  ...
  original_markers <- Seurat::FindMarkers(orig_seurat_obj,
  ...
  markers_selected <- compute_knockoff_filter(knock_seurat_obj,
                                              knock_ident_1,
                                              knock_ident_2,
                                              q)

  thres <- markers_selected$threshold
  markers_selected <- markers_selected$selected_features

  top_original <- rownames(utils::head(original_markers, 50))
  top_knockoffs <- utils::head(markers_selected, 50)$selected_gene

  ret_list <- list("jaccard" = jaccard(top_original, top_knockoffs),
                   "original_selected" = dim(original_markers)[1],
                   "knockoff_selected" = dim(markers_selected)[1],
                   "intersection" = length(intersect(top_original, top_knockoffs)),
                   "top_original" = top_original,
                   "top_knockoffs" = top_knockoffs
                   )
  return(ret_list)
}
...
cluster_seurat_heuristically_find_num_clusters_hierarchical_clustering <- function(seurat_obj,
  ...
  all_genes <- rownames(knockoff_seurat_obj)
  ...
  while (TRUE) {
    ...
    for (i in 1:length(knock_idents)) {
      ...
      if (found_no_sign_diff) {
        print("Found clusters with no significant differences.")
        print("Progressing to next clustering iteration.")
        break
      }
    }
...
cluster_seurat_heuristically_find_num_clusters <- function(seurat_obj,
  ...
  # bump up starting point by the decrement because we subtract the decrement
  # in the loop
  ...
  while (TRUE) {
    ...
    if (algorithm == "leiden") {
      ...
      #plan("sequential")
      # todo log number of cores being used
      # this is a weird one because leiden has a forked job hanging
      knockoff_seurat_obj <- Seurat::FindClusters(knockoff_seurat_obj,
                                                  resolution = resolution_param,
                                                  algorithm = 4,
                                                  method = "igraph")
...
get_cluster_centroid_distance_matrix <- function(seurat_obj, num_PCs) {
  ...
  centroids <- lapply(cluster_names,
                      FUN = function(cluster_name) {
                        get_cluster_centroid(seurat_obj, num_PCs, cluster_name)
                      })

  centroid_df <- t(as.data.frame(centroids))
  rownames(centroid_df) <- cluster_names


  distance_mat <- stats::dist(centroid_df)
...
get_cluster_comparison_order_sorted_by_centroid_distance <- function(seurat_obj, num_PCs) {
  ...
  dist_mat <- get_cluster_centroid_distance_matrix(seurat_obj, num_PCs)

  unsorted_comparisons <- unlist(lapply(1:num_comparisons, function(comparison) { dist_mat[comparisons[1, comparison], comparisons[2, comparison]] }))
